...
show_menu() {
    ...
    echo "  - Clicking it will run: /Applications/Antigravity.app/Contents/MacOS/Electron . --remote-debugging-port=9000"
    echo ""
    echo "REQUIREMENTS:"
    echo "  - Nautilus file manager (GNOME)"
    echo "  - Desktop Editor must be installed and in your PATH"
    echo ""
...
while true; do
    ...
    case $choice in
        1)
            ...
            echo "/Applications/Antigravity.app/Contents/MacOS/Electron . --remote-debugging-port=9000" >> "$SCRIPT_FILE"
            chmod +x "$SCRIPT_FILE"
            echo ""
        ...
        4)
            ...
            if [ -f "$SCRIPT_FILE" ]; then
                mkdir -p "$BACKUP_DIR"
                BACKUP_FILE="$BACKUP_DIR/nexus_backup_$(date +%Y%m%d_%H%M%S).sh"
